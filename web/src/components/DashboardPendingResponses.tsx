...
function DashboardPendingResponses() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to load pending responses data');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) return (
    <div className="card">
      <div className="card-body flex flex-col items-center gap-2 py-8">
        ...
        <p className="text-gray-600 text-sm">Loading pending responses...</p>
      </div>
    </div>
  );
  ...
  if (!loading && pendingResponses.length === 0) return null;
  ...
  const tabs = [
    ...
    { key: 'essays', label: `Essays${essays.length > 0 ? ` (${essays.length})` : ''}` },
  ] as const;

  return (
    <div className="card">
      <div className="card-header">
        <h3 className="font-semibold text-brand-700">Pending Responses</h3>
      </div>
      <div className="card-body">
        ...
        {activeTab === 'recommendations' && renderList(recommendations, 'No pending recommendation responses.', '📝')}
        {activeTab === 'essays' && renderList(essays, 'No pending essay review responses.', '✏️')}
      </div>
    </div>
  );
}
